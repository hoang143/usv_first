...
void loop(){
  ...
  { 
    if (Udp.remoteIP() == laptop)
    {
      ...
      if (incomingPacket[0] == 37)
      {
        ...
        zigzag_step = parse_integer_3_digit(28);
  ...
  if (select_auto_mode == 2)
  {
    ...
    if (((millis()-arrival_start_time) > 1000)&&(arrival_prev>0))
    {
      arrival_start_time = millis();
      switch_target = true;
    }
  ...
  if ((auto_mode>0)&&(select_auto_mode == 3))
  {
    ...
    if (reach_A>0)
    {
      desire_yaw = get_look_ahead_yaw(usv_lat, usv_lng, target_lat_1, target_lng_1, target_lat_2, target_lng_2);
    }
  }
...
//
  ...
  if ((auto_mode > 0)&&((select_auto_mode == 1)||(select_auto_mode == 2)||((select_auto_mode == 3)&&(reach_A<1))))
...
// ----------------------------------------------- THRUST AND MOMENT -----------------------------------------------
  ...
  if (auto_mode < 1)
  {
    ...
    moment = moment = k_moment * yaw_error;
...
// ----------------------------- Control PWM -----------------------------
  ...
  speed_1 = thrust + moment + 10; // PWM output, range 0-255
  speed_2 = thrust - moment - 10;
  if (speed_1>190) speed_1 = 190;
