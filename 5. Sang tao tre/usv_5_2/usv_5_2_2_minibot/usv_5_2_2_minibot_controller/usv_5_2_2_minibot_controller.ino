...
// OBSTACLE AVOIDANCE
  ...
  const float side_avoid_limit = 0.1;
  const float side_avoid_track_radius = 0.4;
  const float side_avoid_detect_radius = 0.7;

  const float front_avoid_limit = 0.4;
  const float front_avoid_detect_radius = 0.9;
...
// ------------ OBSTACLE AVOIDANCE: Desire yaw adjustment ------------
...
// ----------------------------------------------- THRUST AND MOMENT -----------------------------------------------
  ...
  // ------------ OBSTACLE AVOIDANCE: Thrust adjustment ------------
  // Detect and Limit thrust 
  if (distance_front < front_avoid_detect_radius)
  {
    thrust = thrust * (distance_front - front_avoid_limit)/(front_avoid_detect_radius - front_avoid_limit);
  }
...
// ----------------------------- CONTROL PWM -----------------------------
  ...
  speed_1 = thrust + moment; // PWM output, range 0-255
  speed_2 = thrust - moment;
  if (speed_1>250) speed_1 = 250;
  if (speed_2>250) speed_2 = 250;
  ...
  //f_1.write(speed_1);
  //f_2.write(speed_2);
  analogWrite(pin_motor_1, speed_1);
  analogWrite(pin_motor_2, speed_2);
