...
// CONTROL PARAMETER
  ...
  const float a = 2;
  const float b = 20;
  const float k_moment = 175/pi;
  const float k_moment_2 = 300/pi;
  
  const float cruise = 190;
...
void loop(){
  ...
  for (int i= 1; i<15; i++)
  {
    if (Udp.remoteIP() == laptop)
    {
        ...
        {
          auto_mode = 0;
          thrust = parse_integer_3_digit(3);
          moment = parse_integer_3_digit(8);
