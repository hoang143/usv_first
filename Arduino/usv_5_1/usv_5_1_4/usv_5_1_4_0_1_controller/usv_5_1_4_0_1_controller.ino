...
// CONTROL PARAMETER
  ...
  const float k_moment_2 = 200/pi;
  const float cruise = 230;
  const float arrival_radius = 2;
...
// ----------------------------------------------- CONTROL FACTORS-----------------------------------------------    
  ...
  Serial.print("; ");
  fuzzy_low = fuzzy_member(1, yaw_error, -pi/12, 0, pi/12);
  fuzzy_mid = fuzzy_member(2, yaw_error, -pi/12, 0, pi/12);
  fuzzy_high = fuzzy_member(3, yaw_error, -pi/12, 0, pi/12);
...
// ----------------------------- Control PCA9685 PWM Driver-----------------------------
  ...
  speed_1 = thrust + moment + 5; // PWM output, range 0-255
  speed_2 = thrust - moment - 5;
